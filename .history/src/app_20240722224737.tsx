  ...
  const [completedAchievements, setCompletedAchievements] = useState<Achievement[]>([]);
	...
	return (
		<main className="bg-slate-900 text-white min-h-screen flex flex-col">
          ...
          <button onClick={() => setIncompleteOpen(!incompleteOpen)}>Incomplete</button>
          <button onClick={() => setCompletedOpen(!completedOpen)}>Completed</button>
				</div>
				<div className="flex flex-col gap-4 mt-4 max-w-[600px] mx-auto">
					{incompleteOpen && incompleteAchievements.map((achievement: Achievement) => (
						<div
        ...
        <div className="flex flex-col gap-4 mt-4 max-w-[600px] mx-auto ">
          <h2 className="text-xl font-bold">Completed</h2>
				...
				<div className="flex flex-col gap-4 mt-4 max-w-[600px] mx-auto ">
					{completedOpen && completedAchievements.map((achievement: Achievement) => (
						<div
							key={achievement.index}
							className="bg-slate-800 p-4 rounded-md"
						>
